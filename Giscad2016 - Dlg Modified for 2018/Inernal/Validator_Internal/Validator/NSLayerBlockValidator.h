...
struct IDCollector{std::vector<long> nObjectIds;};
typedef std::map<NSSTRING, IDCollector*> STRING_ID_TYPE;
typedef std::map<long,long> ID_TYPE;
...
//	int checkRelation(CNSValidationDataCollector* pCollector , IDCollector &nonExtCollector, VALIDATE_COL_MAP_TYPE &mapOfBlocks, NSSTRING strBlkName, long &nObjID, RET_TYPE enLayerType);
	int checkRelation(std::vector<CNSValidationDataCollector*> &vectExtCollector , IDCollector &nonExtCollector, VALIDATE_COL_MAP_TYPE &mapOfBlocks, long &nObjID, RET_TYPE enLayerType);
