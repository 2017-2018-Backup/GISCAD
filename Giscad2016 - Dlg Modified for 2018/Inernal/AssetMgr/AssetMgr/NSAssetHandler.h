...
typedef std::vector<long> OBJECTID_VECTOR ;
...
private:
	...
	int scanServicePillars(std::map<long, ASSET_VECTOR*> & mapOfPillars);
	...
	CNSAsset* getAsset(long nObjectID);
...
private:
	...
	std::map<long, NSSTRING> mapOfNumAssets;
	std::map<long, NSSTRING> m_mapOfEquipNoIDs;
//	std::map<NSSTRING, NSSTRING> m_mapOfEquipNoAssets;
	std::map<long, SWITCH_MAP*> mapofSwitches;
	std::map<long, ASSET_VECTOR*> mapOfPillars;
	std::map<long, NSSTRING> m_mapOfSwitchType;
};
