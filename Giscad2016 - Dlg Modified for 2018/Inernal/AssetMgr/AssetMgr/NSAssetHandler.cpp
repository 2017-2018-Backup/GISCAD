...
int CNSAssetHandler::generateNumbers()
{
	...
	std::map<long, SWITCH_MAP*>::iterator switchItr;
	...
	try
	{
		...
		for(itr = m_mapOfAssets.begin(); itr != m_mapOfAssets.end(); itr++)
		{
			...
			for(vectItr = vectAssets->begin(); vectItr != vectAssets->end(); vectItr++)
			{
				...
				if(NSSTRCMP(strAssetNumber.c_str(), _T("")) != 0)
				{
					mapOfNumAssets.insert(std::map<long,NSSTRING>::value_type(pAsset->getObjectID(), strAssetNumber));
					continue;
				}
				...
				//	NSSTRCPY (szEquipNumber, _T(""));
					...
					/*	TCHAR* szPlantNo = pDataItem->getValueFromMap("PlantNo");
						...
						m_mapOfEquipNoIDs.insert(std::map<long, NSSTRING>::value_type(pAsset->getObjectID(), szEquipNumber));
						delete []szEquipNo;
						///OP_ID implementation
					}
	...
	/*	int nIDCntr = 0;
		...
		std::map<long, NSSTRING>::iterator theNumIterator;
...
int CNSAssetHandler::scanSubStationAssets(std::map<long, SWITCH_MAP*> & mapofSwitches)
{	
	std::map<long, ASSET_VECTOR*> mapOfSubStns;
	std::map<NSSTRING, ASSET_VECTOR*>::iterator assetsItr;
	std::map<NSSTRING, ASSET_VECTOR*>::iterator assetsSubSwicthesItr;
	std::map<long, int> mapOfMatchingSwitches;
	std::map<long, int> mapOfInvalidSubSwitches;
	std::map<long, int>::iterator InvalidSubSwitchItr;
	...
	if(assetsSubSwicthesItr != m_mapOfAssets.end())
	{
		...
		for(subSwitchesItr = subSwitchesAssets->begin(); subSwitchesItr != subSwitchesAssets->end(); subSwitchesItr++)
		{
			...
			if(nResult == NS_FAIL)
			{
				mapOfInvalidSubSwitches.insert(std::map<long, int>::value_type(pSwitchAsset->getObjectID(), 1));
			}
		}
	}
	...
	if(assetsItr != m_mapOfAssets.end())
	{
		...
		for(subStnItr = subStnAssets->begin(); subStnItr != subStnAssets->end(); subStnItr++)
		{
				...
				if(assetsItr != m_mapOfAssets.end())
				{
					...
					for(subSwitchItr = pSubSwitchVect->begin(); subSwitchItr != pSubSwitchVect->end(); subSwitchItr++)
					{
						...
						if(switchAttItr != mapOfSwitchAtts.end())
						{
							...
							{
								std::map<long, ASSET_VECTOR*>::iterator subStnItr;
								...
								else
								{
									...
									mapOfSubStns.insert(std::map<long, ASSET_VECTOR*>::value_type(pAsset->getObjectID(), switchVect));
								}
								mapOfMatchingSwitches.insert(std::map<long, int>::value_type(pSubSwitch->getObjectID(), 0));
							}
						}
						//else
						//{
			...
			std::map<long, ASSET_VECTOR*>::iterator tempIter;
	...
	for(subStnItr = mapOfSubStns.begin(); subStnItr != mapOfSubStns.end(); subStnItr++)
	{
		...
		long lSubStnID = (*subStnItr).first;
		...
		for(switchItr = switchVect->begin(); switchItr != switchVect->end(); switchItr++)
		{
			...
			//	NSSTRING strSwitchType = (*attrItr).second;
				...
				{
					m_mapOfSwitchType.insert(std::map<long, NSSTRING>::value_type(pSwitch->getObjectID(),  (*attrItr).second));
				}
		...
		mapofSwitches.insert(std::map<long, SWITCH_MAP*>::value_type(lSubStnID, pSwitchMap));
	}
	...
	if(assetsItr != m_mapOfAssets.end())
	{
		...
		for(subSwitchItr = pSubSwitchVect->begin(); subSwitchItr != pSubSwitchVect->end(); subSwitchItr++)
		{
			std::map<long, int>::iterator subMatchingSwitchItr;
...
int CNSAssetHandler::scanServicePillars(std::map<long, ASSET_VECTOR*> & mapOfPillars)
{
	std::map<NSSTRING, ASSET_VECTOR*>::iterator assetsItr;
	std::map<long, int> mapOfMatchingServicePillars;
	std::map<NSSTRING, ASSET_VECTOR*>::iterator assetsServicePillarsItr;
	std::map<long, int> mapOfInvalidServicePillars;
	std::map<long, int>::iterator InvalidServicePillarsItr;
	...
	if(assetsServicePillarsItr != m_mapOfAssets.end())
	{
		...
		for(servicePillarItr = subServicePillarAssets->begin(); servicePillarItr != subServicePillarAssets->end(); servicePillarItr++)
		{
			...
			if(nResult == NS_FAIL)
			{
				mapOfInvalidServicePillars.insert(std::map<long, int>::value_type(pServicePillarAsset->getObjectID(), 1));
			}
		}
	}
		...
		for(lvPillarAssetsItr = lvPillarAssets->begin(); lvPillarAssetsItr != lvPillarAssets->end(); lvPillarAssetsItr++)
		{
			...
			if(attributesItr != mapofattributes.end())
			{
				...
				if(assetsItr != m_mapOfAssets.end())
				{
					...
					for(servicePillarVectItr = pServicePillarVect->begin(); servicePillarVectItr != pServicePillarVect->end(); servicePillarVectItr++)
					{
						...
						if(servicePillarItr != mapOfServicePillarAtts.end())
						{
							...
							{
								std::map<long, ASSET_VECTOR*>::iterator pillarsItr;
								...
								else
								{
									...
									pServiceVect->push_back(pServicePillar);
									mapOfPillars.insert(std::map<long, ASSET_VECTOR*>::value_type(pAsset->getObjectID(), pServiceVect));
								}
								mapOfMatchingServicePillars.insert(std::map<long, int>::value_type(pServicePillar->getObjectID(), 0));
							}
						}
					}
				}
				else
				{
					...
					mapOfPillars.insert(std::map<long, ASSET_VECTOR*>::value_type(pAsset->getObjectID(), pVect));
				}
			}
		}
	...
	if(assetsItr != m_mapOfAssets.end())
	{
		...
		{
			std::map<long, int>::iterator subMatchingPillarItr;
...
void CNSAssetHandler::generateAssetsForPillars(std::map<TCHAR*, TCHAR*> & mapofCredentials)
{
	...
	for(pillarsItr = mapOfPillars.begin(); pillarsItr != mapOfPillars.end(); pillarsItr++)
	{
		...
		long pillarObjID = (*pillarsItr).first;
		...
		if(NSSTRCMP(strAssetNumber.c_str(), _T("")) == 0)
		{
			...
			/*	TCHAR* szPlantNo = pDataItem->getValueFromMap("PlantNo");
				...
				m_mapOfEquipNoIDs.insert(std::map<long, NSSTRING>::value_type(pAsset->getObjectID(), szMainPillarEquip));
				delete []szEquip;
...
void CNSAssetHandler::generateAssetsForSwitches(std::map<TCHAR*, TCHAR*> & mapofCredentials)
{
	...
	for(switchItr = mapofSwitches.begin(); switchItr != mapofSwitches.end(); switchItr++)
	{
		...
		long lSubStnID = (*switchItr).first;
		...
		if(NSSTRCMP(strAssetNumber.c_str(), _T("")) == 0)
		{
			...
			/*	TCHAR* szPlantNo = pDataItem->getValueFromMap("PlantNo");
				...
				m_mapOfEquipNoIDs.insert(std::map<long, NSSTRING>::value_type(pSubStnAsset->getObjectID(), szSubEquipNo));
				delete []szEquipNo;
		...
		{
			//mapOfNumAssets.insert(std::map<long,int>::value_type(pSubStnAsset->getObjectID(), 1));
			mapOfNumAssets.insert(std::map<long,NSSTRING>::value_type(pSubStnAsset->getObjectID(), strAssetNumber));
		...
		if(EquipNoAssetsItr != mapOfValues.end())
		{
				...
				if(nResult == NS_SUCCESS)
				{
					...
					if(pAsset)
					{
						...
						int nFNO = pAsset->getFNO();
						mapOfNumAssets.insert(std::map<long,NSSTRING>::value_type((*EquipNoIDsItr).first, (*EquipNoAssetsItr).second));
						TCHAR szMsg[1000];												
...
{
	std::map<long, SWITCH_MAP*>::iterator switchItr;
	std::map<long, NSSTRING>::iterator AssetNumItr;
	...
	for(switchItr = mapofSwitches.begin(); switchItr != mapofSwitches.end(); switchItr++)
	{
		...
		long lSubStnID = (*AssetNumItr).first;
		CNSAsset *pAsset = getAsset(lSubStnID);
		NSSTRING strSubStnAssNo = (*AssetNumItr).second;
		SWITCH_MAP::iterator itA;

		
		std::map<long, NSSTRING>::iterator theSwitchTypeItr;
		for(itA = switchMap->begin(); itA != switchMap->end(); itA++)
		{
		...
		//	NSSTRING strSwitchType = (*itA).first;
			...
			/*	TCHAR* szPlantNo = pDataItem->getValueFromMap("PlantNo");
				...
				m_mapOfEquipNoIDs.insert(std::map<long, NSSTRING>::value_type(pAsset->getObjectID(), szSubSwitchEquipNo));
			}
			...
			for(it10 = pObjIdVect->begin(); it10 != pObjIdVect->end(); it10++)
			{
				...
				if(iRet != -1)
				{
					...
					WriteToLog(LOG_HEADER, szMsg);
					//mapOfNumAssets.insert(std::map<long,int>::value_type(objID, 1));
					mapOfNumAssets.insert(std::map<long,NSSTRING>::value_type(objID, strsubSwitchAssetNo));
...
CNSAsset* CNSAssetHandler::getAsset(long nObjectID)
{
	std::map<NSSTRING, ASSET_VECTOR*>::iterator it;
	CNSAsset *pAsset = NULL;
	for(it = m_mapOfAssets.begin(); it != m_mapOfAssets.end(); it++)
	...
	for(pillarsItr = mapOfPillars.begin(); pillarsItr != mapOfPillars.end(); pillarsItr++)
	{
		//AK END
			...
			for(serviceItr = pServicePillarVect->begin(); serviceItr != pServicePillarVect->end(); serviceItr++)
			{
				...
				if(AssetNumItr == mapOfNumAssets.end()) continue;
				long lPillarID = (*AssetNumItr).first;
			//	CNSAsset *pAsset = getAsset(lPillarID);
				...
				for(dataItemItr = dataItemsVect.begin(); dataItemItr != dataItemsVect.end(); dataItemItr++)
				{
					...
					m_mapOfEquipNoIDs.insert(std::map<long, NSSTRING>::value_type(pServicePillar->getObjectID(), szSPillarEquipNo));
				}
				...
				assignOPIds(mapofCredentials, mapOfValues, true);
				ValueIter = mapOfValues.begin();
				long objID = pServicePillar->getObjectID();
				CNSAsset *pAsset = new CNSAsset;
				pAsset->setObjectID(objID);
				if(ValueIter != mapOfValues.end()) 
				{
					...
					if(iRet != -1)
					{
						...
						WriteToLog(LOG_HEADER, szMsg);
						//mapOfNumAssets.insert(std::map<long,int>::value_type(objID, 1));
						mapOfNumAssets.insert(std::map<long,NSSTRING>::value_type(objID, strServicePillarAssetNo));
