...
public:
	...
	int getFeaturesToBeValidated(std::map<long, CNSValidationDataCollector*> &mapBlocksToBeValidated, std::map<long, CNSValidationDataCollector*> &mapLinearFeaturesToBeValidated, bool bGetXInfo, bool getLinerFeatures, TCHAR* pszLayerNameSuffix);//, std::map<long, AcDbObjectId> &tempReferenceStorage);
