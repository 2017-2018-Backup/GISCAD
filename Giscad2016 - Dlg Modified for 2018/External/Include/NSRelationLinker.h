...
private:
	...
	// Utility functions
	CNSValidationDataCollector* getFeature(long lObjectId); // gets feature from m_mapBlocksToBeLinked or m_mapLinearFeaturesToBeLinked
...
public:
	std::map<long, CNSProposedRelations*> m_mapProposedRelations;			// this information is to be sent to postXDATA 
private:
	...
	std::map<long, CNSValidationDataCollector*> m_mapBlocksToBeLinked;		// NEW and MOD blocks are stored in this map
	std::map<long, CNSValidationDataCollector*> m_mapLinearFeaturesToBeLinked; // NEW linear features(i.e. cables) are stored in this map
	std::map<long, CNSValidationDataCollector*> m_mapExtBlocks;				// Only EXT Blocks
	std::map<long, CNSValidationDataCollector*> m_mapExtCablesToBeLinked;	// Only EXT cables
};
