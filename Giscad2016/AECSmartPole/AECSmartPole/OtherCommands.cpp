...
void ModifyPoleTable(AcDbObjectId objPole, CString csPoleNumber, CString csNewPoleNumber)
{
  ...
  acutRelRb(rbpPoleXD);
	...
	ads_name enTable;
	long lLength = 0L; acedSSLength(ssTables, &lLength);
...
void Command_ResizePoleText()
{
  ...
  long lLength = 0L; if ((acedSSLength(ssPoles, &lLength) != RTNORM) || (lLength == 0L)) { acedSSFree(ssPoles); return; }
...
int GetCablesAttachedToPole(AcDbObjectId objPole, CStringArray &csaCableHandles)
{
	...
	long lLength = 0L; acedSSLength(ssCables, &lLength);
...
{ 
	...
	{
		long lClean = 0L;	acedSSLength(ssClean, &lClean);
		...
		for (long lCtr = 0; lCtr < lClean; lCtr++)
		{
	...
	for (long lCtr = 0L; lCtr < lLength; lCtr++)
	{
		...
		long lLWLength = 0L; acedSSLength(ssLWCables, &lLWLength);
		...
		for (long lCnt = 0L; lCnt < lLWLength; lCnt++)
		{
			...
			ads_name enExSeg;
			long lExSegLen = 0L; acedSSLength(ssCables, &lExSegLen);
			for (long lExSeg = 0L; lExSeg < lExSegLen; lExSeg++)
			{
...
void Command_AddBond()
{
	...
	if (ilongHndlInd == 2)
	{
		...
		long lCables = 0L; acedSSLength(ssCables, &lCables);
