...
int CNSDWGMgr::addXDATA(long lObjectId, std::map<NSSTRING, NSSTRING> &mapXData)
{
	...
	if(lObjectId<=0) 
	{
		...
		for (pBlkTblRcdItr->start(); !pBlkTblRcdItr->done(); pBlkTblRcdItr->step())
		{
			...
			{
				if(((long)(pEnt->id().asOldId())) == lObjectId)
				{				
...
*/
int CNSDWGMgr::getFeaturesToBeValidated(std::map<long, CNSValidationDataCollector*> &mapBlocksToBeValidated, std::map<long, CNSValidationDataCollector*> &mapLinearFeaturesToBeValidated, bool bGetXInfo, bool getLinerFeatures, TCHAR* pszLayerNameSuffix)//,std::map<long, AcDbObjectId> &tempReferenceStorage)
{
