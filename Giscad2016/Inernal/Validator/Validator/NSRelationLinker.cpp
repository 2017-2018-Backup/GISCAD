...
extern std::map<long,long> gMapOfValidIDs;
...
CNSRelationLinker::~CNSRelationLinker(void)
{
	...
	std::map<long, CNSProposedRelations*>::iterator mapItr;
	...
	std::map<long, CNSValidationDataCollector*>::iterator blockMapItr;
	...
	std::map<long, CNSValidationDataCollector*>::iterator cableMapItr;
	...
	std::map<long, CNSValidationDataCollector*>::iterator extMapItr;
...
int CNSRelationLinker::initializeFeatures(bool bProposedToExisting, int LinearConnOnLayerFlag)
{
	...
	std::map<long, CNSValidationDataCollector*>::iterator valDataItr;	
...
{
	std::map<long, CNSValidationDataCollector*>::iterator featuteItr;
...
void CNSRelationLinker::establishRelationship(bool bProposedToExisting, int LinearConnOnLayerFlag)
{
	...
	std::map<long, CNSValidationDataCollector*>::iterator blkMapItr;	
...
{
	std::map<long, CNSValidationDataCollector*>::iterator FeatureItr1, FeatureItr2, FeatureItr3, FeatureItr4, FeatureItr5, FeatureItr6, FeatureItr7, FeatureItr8, FeatureItr9, FeatureItr10;
...
{
	int nFID = -1;
	std::map<long, CNSValidationDataCollector*>::iterator blocksToBeLinkedItr, cablesToBeLinkedItr, extBlockItr, extCableItr;
...
{
	std::map<long, CNSValidationDataCollector*>::iterator BlocksToBeLinkedItr;
	std::vector<double> vectDist;

	//Get the vectors of objectids from the CNSValidationDataCollector class
	std::vector<long> vectFeaturesInProximity = ValidationDataCollectorObj->m_vectFeaturesInProximity; 
	std::vector<long>::iterator ObjIdIter;
...
void CNSRelationLinker::assignOwnershipParentChild(CNSValidationDataCollector* pFeature)
{
	...
	{
		std::vector<long>::iterator vectProxItr;
		...
		for(vectProxItr = pFeature->m_vectFeaturesInProximity.begin(); vectProxItr != pFeature->m_vectFeaturesInProximity.end(); vectProxItr++)
		{
			...
			{
				long nProxObjectId = (*vectProxItr);
	...
	if(nParentFID != nNumberNotFound)
	{
		...
		//
		long lObjectId = pFeature->getObjectId();
		std::map<long, CNSProposedRelations*>::iterator mapItr; //m_mapProposedRelations 
		...
		//
		long lParentObjectId = pParentFeature->getObjectId();		
...
*/
CNSValidationDataCollector* CNSRelationLinker::getFeature(long lObjectId)
{
	std::map<long, CNSValidationDataCollector*>::iterator mapItr;
...
{	
	std::map<long, long> mapOfInvalidIds;
	std::map<long, long>::iterator mapValidIdItr, mapInvalidIdItr;
	std::map<long, CNSValidationDataCollector*>::iterator mapPropBlockItr, mapCableItr, mapExtBlkItr, mapExtCableItr;
	...
	{
		long lBlkId = (*mapPropBlockItr).first;
		mapValidIdItr = gMapOfValidIDs.find(lBlkId);
		if(mapValidIdItr == gMapOfValidIDs.end())
		{
			mapOfInvalidIds.insert(std::map<long, long>::value_type(lBlkId, nNumberNotFound)); // second value in yhe map is dummy and is not used anywhere
		}
	}
	for(mapCableItr = m_mapLinearFeaturesToBeLinked.begin(); mapCableItr != m_mapLinearFeaturesToBeLinked.end(); mapCableItr++)
	{
		long lBlkId = (*mapCableItr).first;
		mapValidIdItr = gMapOfValidIDs.find(lBlkId);
		if(mapValidIdItr == gMapOfValidIDs.end())
		{
	...
	{
		long lBlkId = (*mapExtBlkItr).first;
		mapValidIdItr = gMapOfValidIDs.find(lBlkId);
		if(mapValidIdItr == gMapOfValidIDs.end())
		{
			mapOfInvalidIds.insert(std::map<long, long>::value_type(lBlkId, nNumberNotFound)); // second value in yhe map is dummy and is not used anywhere
	...
	{
		long lBlkId = (*mapExtCableItr).first;
		mapValidIdItr = gMapOfValidIDs.find(lBlkId);
		if(mapValidIdItr == gMapOfValidIDs.end())
		{
			mapOfInvalidIds.insert(std::map<long, long>::value_type(lBlkId, nNumberNotFound)); // second value in yhe map is dummy and is not used anywhere
...
{
	std::map<long, CNSValidationDataCollector*>::iterator blocksToBeLinkedItr, cablesToBeLinkedItr;
	CNSValidationDataCollector* pValidationDataCollectorObj;
	...
	for(blocksToBeLinkedItr = m_mapBlocksToBeLinked.begin(); blocksToBeLinkedItr != m_mapBlocksToBeLinked.end(); blocksToBeLinkedItr++)
	{
		...
		m_mapProposedRelations.insert(std::map<long, CNSProposedRelations*>::value_type(pValidationDataCollectorObj->getObjectId(), pTempProposedRel));
	}
...
void  CNSRelationLinker::clearExistingXDataForNew()
{
	...
	for(valDataItr = m_mapBlocksToBeLinked.begin(); valDataItr != m_mapBlocksToBeLinked.end(); valDataItr++)
	{
		...
		if(pTempFeature)
		{
			long lObjectId = pTempFeature->getObjectId();
			STRING_MAP mapXDATA;
